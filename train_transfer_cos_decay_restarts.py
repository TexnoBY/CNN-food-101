...
def build_model():
  ...
  x = tf.keras.applications.EfficientNetB0(include_top=False, weights='imagenet', input_tensor=inputs)
  x.trainable = False
...
def main():
  ...
  cos_decay = tf.keras.experimental.CosineDecayRestarts(initial_learning_rate = 0.001,
				                        first_decay_steps = 1000)
  learning_rate = LearningRateScheduler(cos_decay)
  model.compile(
  ...
  log_dir='{}/f101-cos_decay_restarts-{}'.format(LOG_DIR, time.time())
  model.fit(
    train_dataset,
    epochs=50,
